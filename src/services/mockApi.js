...
export async function generateRecipe({ ingredients, diet, cuisine, time, difficulty }) {
  await delay(900);
  const have = ingredients.map((i) => i.trim()).filter(Boolean);
  const haveLower = have.map((i) => i.toLowerCase());

  // First, see if a curated recipe already fits what was asked for — matching
  // on cuisine and ingredient overlap. Real /generate would let the AI model
  // decide this; here we score against the mock catalog so obvious matches
  // (e.g. "Rajma" + North Indian) return the real "Rajma Chawal" recipe
  // instead of a generic fabricated one.
  const scored = mockRecipes
    .filter((r) => !cuisine || r.cuisine.toLowerCase() === cuisine.toLowerCase())
    .map((r) => {
      const nameHit = haveLower.some(
        (h) => r.title.toLowerCase().includes(h) || h.includes(r.title.toLowerCase())
      );
      const ingredientHits = r.ingredients.filter((ing) =>
        haveLower.some((h) => ing.name.toLowerCase().includes(h) || h.includes(ing.name.toLowerCase()))
      ).length;
      return { recipe: r, score: (nameHit ? 5 : 0) + ingredientHits };
    })
    .filter((s) => s.score > 0)
    .sort((a, b) => b.score - a.score);

  if (scored.length > 0) {
    const match = scored[0].recipe;
    history = [
      { id: `h${Date.now()}`, recipeId: match.id, generatedAt: new Date().toISOString() },
      ...history,
    ];
    return match;
  }

  // No good match — fall back to a synthesized recipe so there's still
  // something reasonable to show. This becomes a real AI-authored recipe
  // once the /generate endpoint is wired to OpenAI/Claude.
  const generated = {
    id: `r${Date.now()}`,
    title: have.length ? `${have[0]} ${cuisine || "Fusion"} Bowl` : "Pantry Surprise Bowl",
    cuisine: cuisine || "Indian",
